  ...
  return (
    <div className="rounded-xl bg-white p-4 shadow-card">
      ...
      <div className="mt-2 flex flex-col">
        {top5.map(function (d) {
          return (
            <div key={d.ean} className="border-b border-line/50 py-1.5 last:border-b-0">
              <div className="text-[9px] font-mono text-[#94A3B8]">{d.ean}</div>
              <div className="truncate text-[8px] text-[#6B7280]" title={d.description}>
                {d.description}
              </div>
              <div className="mt-0.5 flex items-center justify-between gap-2">
                <span className="truncate text-[9px] text-[#2563EB]">{d.category ? d.category : "Sem categoria"}</span>
                <span className="shrink-0 text-[12px] font-bold text-[#DC2626]">{d.diffPct}%</span>
              </div>
            </div>
          );
        })}
