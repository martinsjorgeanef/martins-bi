...
function SortHeader({
  ...
  return (
    <th
      ...
      className={clsx(
        "cursor-pointer select-none whitespace-nowrap px-3 py-2.5 text-[13px] font-semibold uppercase tracking-wide text-ink-600 hover:text-ink-950",
    ...
    >
      ...
      >
        {label}
        <ArrowUpDown size={10} className={active ? "text-accent" : "text-ink-500/40"} />
      </span>
    </th>
  );
}
        ...
        <table className="w-full table-fixed border-collapse">
          ...
          <thead className="sticky top-0 border-b border-line bg-surface">
            <tr>
              ...
              <th className="whitespace-nowrap px-3 py-2.5 text-left text-[13px] font-semibold uppercase tracking-wide text-ink-600">
                Categoria
              </th>
              ...
              <th className="whitespace-nowrap px-3 py-2.5 text-center text-[13px] font-semibold uppercase tracking-wide text-ink-600">
                Status
              </th>
            </tr>
          </thead>
              ...
              ? rows.map(function (r) {
                  return (
                    <tr key={r.id} className="border-b border-line/50 hover:bg-surface/60">
                      <td className="truncate px-3 py-1.5 font-mono text-[12px] text-ink-700">{r.ean}</td>
                      <td className="truncate px-3 py-1.5 text-[12px] text-ink-950" title={r.description}>
                        {r.description}
                      </td>
                      <td className="truncate px-3 py-1.5 text-[12px] text-ink-700" title={r.category || ""}>
                        {r.category ? r.category : "-"}
                      </td>
                      <td className="whitespace-nowrap px-3 py-1.5 text-right text-[12px] font-medium tabular-nums text-ink-950">
                        {money(r.martinsPrice)}
                      </td>
                      <td className="whitespace-nowrap px-3 py-1.5 text-right text-[12px] tabular-nums text-ink-700">
                        {r.marketPrice !== null ? money(r.marketPrice) : "-"}
                      </td>
                      <td className="truncate px-3 py-1.5 text-[12px] font-medium text-ink-700" title={r.bestCompetitor || ""}>
